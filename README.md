...
| Phase 2 Sprint 3: collector ingestion adapter | Bridge Collector source payloads into the shared station event model without breaking ACK, storage or production fact authority. | Advance in small slices: offline adapter, mapping hardening, runtime adapter gate, diagnostics, raw boundary tests, decoder authority, runtime raw wiring, then narrow runtime raw decoder repair. | Slice E1 runtime raw decoder repair is closed and committed at `2c73410`; DB/API/Dashboard/V-PLC/storage.py/ACK/deploy surfaces remain unchanged unless separately authorized. |
...
authority when it conflicts with the status documents. E1 closes the narrow
Snap7 runtime raw decoder buffer repair after D3; DB/API/Dashboard/V-PLC,
`storage.py`, ACK/read_done ownership or deploy changes still require separate
PM authorization.
