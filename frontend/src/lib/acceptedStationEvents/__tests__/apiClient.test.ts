...
function expectSingleAcceptedRequest(fetchMock: ReturnType<typeof vi.fn>) {
  ...
  const requestUrl = new URL(fetchMock.mock.calls[0][0] as string, "http://dashboard.test");
...
describe("accepted station events api client", () => {
  ...
  it.each([
    ...
    expect(await fetchAcceptedStationEvents(validQuery, fetchMock)).toMatchObject({ ok: false, kind: "invalid-query" });
    expectSingleAcceptedRequest(fetchMock);
  });
  ...
  it("maps accepted fact source unavailable to unavailable without fallback", async () => {
    ...
    expect(await fetchAcceptedStationEvents(validQuery, fetchMock)).toMatchObject({ ok: false, kind: "unavailable" });
    expectSingleAcceptedRequest(fetchMock);
  });
