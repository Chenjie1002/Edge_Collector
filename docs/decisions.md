...
## ADR-014: 树莓派作为 Edge 节点，不承担高频采样和长期重计算

决策：

- 树莓派负责 Edge SCADA / MES 节点能力。
- 高频原始信号采集由 MCU 或专用采集器完成。
- MCU 每个零件生成一个 CSV 或 JSON 文件。
- Edge 负责抓取文件、解析、特征提取、追溯关联和归档。
- AI 推理和长期大数据分析放到 Nvidia 边缘设备或服务器。

原因：

- 树莓派 5B 适合采集、落库、追溯、看板和轻量分析。
- 高频采样对实时性、缓冲和抗干扰要求更高，MCU 更合适。
- AI 和长期分析不应挤占产线侧基础采集资源。

影响：

- 后续需要新增 `part_data_file` 和 `signal_feature`。
- 文件命名或文件内容必须包含 `unit_id`、DMC、station_id 或时间戳等关联字段。
- Edge 需要提供文件接入、解析失败重试和归档状态追踪。

## ADR-015: 参数管理只读校验，不主动回写 PLC

决策：

- 参数管理纳入核心扩展范围。
- Edge 对 PLC 参数只读取、校验和记录，不主动回写 PLC。
- PLC 侧参数修改后通知 Edge，Edge 再读取新参数。
- 被授权账号登录后即可修改参数，不需要审批流。
- 系统必须精确记录参数 changelog。

原因：

- 保持 PLC/HMI 侧对工艺参数的最终控制权。
- 降低 Edge 误写 PLC 参数带来的生产风险。
- changelog 可以满足追溯、审计和工程问题定位。

影响：

- 需要新增参数定义、参数快照和参数变更日志。
- 需要记录账号、认证方式、旧值、新值、参数范围、校验结果和来源。

## ADR-016: 工业相机媒体只做短期缓存和归档索引
...
- 图片/视频数据量大，不适合长期放在树莓派本地。
- 追溯需要快速查看近期媒体，但长期保存应进入专用存储。

影响：

- 需要新增 `media_file`。
- 需要实现 retention job。
- 媒体文件应关联 `unit_id`、station_id、camera_id、capture_time 和 inspection_result。

## ADR-017: 归档支持冷热备份并作为可配置任务

决策：

- 归档目标支持移动硬盘冷备份和服务器热备份。
- 两种目标都作为可勾选项。
- 归档周期支持按月或按季度强制执行，并允许后续灵活调整。
...
- 需要新增 `archive_job` 和 `archive_job_item`。
- 归档任务需要记录目标、范围、状态、失败原因、校验结果和重试次数。

## ADR-018: 权限体系预留生物识别和令牌认证
...
- 支持账号密码、令牌验证和生物识别外设。
- 生物识别来源包括人脸识别模块或指纹模块。
- 生物识别作为认证适配器，不直接绑定业务模块。

原因：

- 参数修改、归档、配置变更等动作需要明确责任人。
- 外设认证方式可能随现场硬件变化，不应侵入业务逻辑。

影响：

- 需要新增用户、角色、认证事件和操作审计表。
- 关键操作必须记录账号、认证方式、时间、对象和结果。
