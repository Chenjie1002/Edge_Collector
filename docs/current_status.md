...
本节是当前最高优先级 control block，覆盖 `0K` 中“accepted build/image planning next”的
旧状态词，但保留 `0K` 及更早段落作为历史快照。Authority：用户于
2026-07-30 20:20（UTC+8）授权 ChatGPT PM 执行 docs-only final planning
acceptance/status sync；durable decision：
...
当前 accepted build/image planning contract 为：
...
R60 只替换 R56 中 dependency expected/actual、Config/RootFS oracle、attempt/builder/
candidate ownership、archive/extraction ownership、evidence-root publication/integrity 及其
更具体 HOLD rows。R56 未被明确替换的 exact source、clean-context exclusion、
`linux/arm64`、base/tag/candidate identity、COPY closure、phase separation 与一般
fail-closed intent 继续有效；future executor 不得在 retained R56 与 superseding R60
之间选择更宽松的双重 PASS 路径。
...
R59 的五项有效最小风险已由 R60 scope reset 修复并由 R61 独立关闭，因此历史 HOLD
origin 继续保留，但不再构成 active blocker。当前 review chain：
...
R62/R63 的 non-blocking carry-forward 只在 future execution-preparation Prompt 中冻结：

- ordinary file SHA-256 使用完整 lowercase 64-hex；OCI digest 使用
  `sha256:<64 lowercase hex>`；Git ID、file SHA、OCI digest 与 candidate image ID
  使用不同字段，candidate full ID 不得缩写；
- path domain 明确分为 `repository_relative_path`、`container_absolute_path`、
  `evidence_root_relative_path`；`host_absolute_path` 仅在后续 Prompt 明确授权时使用；
- timestamp 使用可解析 RFC3339 UTC `YYYY-MM-DDTHH:MM:SS[.fraction]Z`，同一 attempt
  且 `start <= end`；不建立 production-time 或 anti-tamper 声明。

这些规则不要求再次修改 R60，也不得扩大为 generic schema registry、SBOM、
supply-chain、audit、forensics 或 retention platform。
...
本次同步时 live `HEAD == origin/main == c3acb33bd089eae4d67aec3be64c97fd128aa178`，
ahead/behind `0/0`；写入前 tracked/cached diff 为空，未跟踪集合为
`309 = Batch D 300 + Batch E 1 + R56–R63 8`，unknown/missing/duplicate `0/0/0`。
本同步只允许创建 R64 并修改本文件与 `docs/roadmap.md`；不执行 stage、commit、push、
tag、Docker、network、build、remote 或 runtime 操作。

下一 eligible branch 是单独授权的 future execution-preparation planning，必须冻结 concrete
attempt ID、local roots、builder/candidate names、source materialization argv、base resolution、
Docker/network budgets、terminal paths、isolated validation commands、stop conditions、
retry/cleanup 与 Git authority。不得从本 control block 直接开始 build。
...
```text
Architecture / Integration six-file implementation: CLOSED / PASS WITH RECOMMENDATIONS
Compose static validation HOLD: CLOSED
...
```text
commit: 8ddba3bd547e9e9bd064b002c150b81324833636
subject: Add Dashboard Raspberry Pi Docker integration
committed paths: exact 10 paths
```

六个 implementation 路径：
...
实现边界：Dashboard 使用 Next standalone、`node:22.23.0-bookworm-slim`、non-root
`node` runtime；Compose 新增独立 `dashboard` service，host `3001` -> container
`3000`；Grafana host `3000` 保持不变；server-side API origin 固定为
`http://api:8000`、profile `container`；`/health` 只证明 Dashboard HTTP process
readiness，不访问 API、DB 或 accepted facts。

本地证据：Node `v22.23.0`；focused health test `1 file / 1 test` PASS；TypeScript
validation PASS；Next production build PASS，包含 `/accepted-events` 与 `/health`；PM
提供的 `docker compose config --quiet` exit `0`；exact target diff check PASS。

证据边界严格保持：上述结果只属于 focused implementation、local source/build 与
Compose static parsing evidence。Docker image build、resolved base digest、final image
ID/size、native `linux/arm64/v8`、Raspberry Pi runtime、port/resource preflight、Compose
startup、rollback/cancellation、真实 Case A/B/C 和 DB/API/Dashboard exact 22-field 三层
闭环均 `NOT EXECUTED / NOT CLAIMED`。

Durable compact status：
...
Gate A closeout 当时的下一 eligible gate 是 Gate B Verification planning review。
该 planning、六文件 implementation、focused review stack 与 exact commit/push 后续已由
Section 0A 记录为 CLOSED。Gate A 本身仍不构成 Docker/Pi/runtime 自动授权。

## 0C. 2026-07-14 Dashboard URL runtime-evidence scope reset
...
该历史分支后续已完成 exact allowlist review 与单独授权的 Git closeout；当前 authority
以 Section 0A 为准。不得重新打开旧 Option C 强审计分支，除非 PM 单独批准为新的
Level 2 子项目。

## 0. 当前 PM / Codex 协作状态

当前主线：D2-R7B-I1 runtime-loaded observability implementation package 已完成 exact-path
Git closeout，accepted build/image planning 已完成 scope reset、Reliability、Data Quality、
Verification review 及 ChatGPT PM final planning acceptance。当前 authority 以 Section 0L
为准；static review stopping rule 已达到，不应继续追加同类静态 review。下一 eligible
branch 仅为单独授权的 future execution-preparation planning，不是直接 build。旧
image-load/activation 记录继续作为历史审计状态，不得作为 commit `934ced7...` 已构建、
部署或加载的证明。Dashboard/UI runtime 验收继续作为最终集成阶段的非阻塞 acceptance debt。
...
Sprint 3 cumulative gate ledger（历史记录；当前 authority 以 Section 0A 为准）：

```text
Dashboard Raspberry Pi Docker Integration Gate B static implementation: CLOSED / PASS WITH RECOMMENDATIONS, commit 8ddba3b
...
Dashboard accepted-events strict parser planning gate: CLOSED / initial HOLD because generic Envelope meta conflicted with endpoint response shape; repaired and closed
Dashboard accepted-events exact-envelope contract clarification: CLOSED / PASS
Dashboard accepted-events exact-envelope Reliability contract review: CLOSED / PASS WITH RECOMMENDATIONS, no blocker
Dashboard accepted-events exact-envelope Data Quality contract review: CLOSED / PASS WITH RECOMMENDATIONS, no blocker
Dashboard accepted-events exact-envelope Verification contract review: CLOSED / initial HOLD for item required/optional and missing/null ambiguity; repaired and closed
Dashboard accepted-events item required-key/null contract repair: CLOSED / PASS
Dashboard accepted-events item required-key/null Data Quality targeted re-review: CLOSED / PASS WITH RECOMMENDATIONS, no blocker
Dashboard accepted-events item required-key/null Verification targeted re-review: CLOSED / PASS WITH RECOMMENDATIONS, original blocker CLOSED
Dashboard accepted-events strict parser implementation: CLOSED / PASS
Dashboard accepted-events strict parser Reliability implementation review: CLOSED / PASS, no blocker
Dashboard accepted-events strict parser Data Quality implementation review: CLOSED / PASS WITH RECOMMENDATIONS, no blocker
Dashboard accepted-events strict parser Verification implementation review: CLOSED / PASS WITH RECOMMENDATIONS, no blocker
Dashboard accepted-events strict parser docs authority exact-path commit/push: PASS, commit 7824063
Dashboard accepted-events strict parser implementation exact-path commit/push: PASS, commit 2cf616d
Dashboard accepted-events strict parser post-push docs/status sync exact-path commit/push: PASS, commit f784c91
...
Dashboard frontend typecheck/build validation planning gate: CLOSED / PASS WITH RECOMMENDATIONS
Dashboard frontend typecheck/build validation planning authority exact-path commit/push: PASS, commit 2a88ffe
Dashboard frontend typecheck/build validation execution: CLOSED / PASS
Dashboard frontend typecheck/build Verification gate: CLOSED / PASS; typecheck/build exit 0, zero tracked frontend drift, generated artifacts cleaned
Dashboard frontend typecheck/build implementation commit: not applicable; commands-only validation changed no source/config/test/package files
...
Dashboard accepted-events strict parser fail-closed hardening is CLOSED by exact-envelope contract clarification, item required-key/null repair and implementation commit `2cf616d`. Docs authority is the separate four-file commit `7824063` (`docs/contracts/dashboard_api_contract.md`, `docs/reports/sprint3_dashboard_strict_parser_fail_closed_plan.md`, `docs/reports/sprint3_dashboard_accepted_events_envelope_contract_clarification_report.md`, `docs/reports/sprint3_dashboard_accepted_events_item_required_key_contract_repair_report.md`); it is not a mixed implementation commit. Implementation commit `2cf616d` changed exactly `frontend/src/lib/acceptedStationEvents/schema.ts`, `frontend/src/lib/acceptedStationEvents/__tests__/schema.test.ts` and `frontend/src/lib/acceptedStationEvents/__tests__/apiClient.test.ts`.

Contract closeout: this endpoint uses an endpoint-specific exact envelope: outer required keys are only `data` and `page`; `data` requires only `items`; `page` requires only `next_cursor` and `limit`; `meta` is unsupported. Envelope/data/page/item unknown keys fail closed. All 22 DTO keys are required by own-property presence; required means the key must exist, not that its value is non-null. Nullable values require explicit JSON `null`; missing is not explicit `null`. Missing and unknown malformed 2xx responses map to client `kind: "error"`. Silent strip, normalization, fallback, partial parsing and nested-field salvage are prohibited. The current API route remains compatible; no API change occurred.

Implementation closeout: `schema.ts` now validates exact enumerable own keys at outer/data/page/item layers and required own-property presence, including the 22-field item missing matrix; `?? null` was removed and explicit `null` is preserved. `apiClient.ts` was not modified; its existing parser-throw mapping remains `kind: "error"`, with no retry, second request or fallback endpoint. No API, DB, Collector, runtime, viewModel, page, store or cache changed. Fresh no-DB focused frontend evidence for the strict-parser implementation branch: `schema.test.ts: 143 passed`; `apiClient.test.ts: 12 passed`; focused diff check: PASS. Full frontend suite, typecheck, build, browser smoke and API/DB tests were not run by that branch.
...
Frontend typecheck/build validation closeout: the Level 1 planning gate is CLOSED / PASS WITH RECOMMENDATIONS and its authority report is committed/pushed at `2a88ffe` / `2a88ffec6dd528b52e104f75f0395f7ecf0bfe2e`. Verification independently executed package-local `npm run typecheck` (`tsc --noEmit`) and `npm run build` (`next build`); both exited 0. The build produced `/_not-found` and `/accepted-events`. There was no tracked frontend drift; `frontend/package.json`, `frontend/package-lock.json`, `frontend/tsconfig.json`, `frontend/next.config.ts` and `frontend/src/**` remained unchanged. Allowed transient artifacts were only `frontend/tsconfig.tsbuildinfo`, `frontend/.next/` and `frontend/next-env.d.ts`; all were precisely cleaned. Final frontend status was empty except pre-existing `frontend/node_modules/`, cached diff was empty, and no tests, install, server, browser, API/DB, Docker or Git write operation ran. Because this was commands-only validation, there is no implementation commit.

The former UI/state stale-data carry-forward, optional all-22-field explicit-null recommendation and current-baseline typecheck/build validation recommendation are CLOSED by commits `156a812`, `963218a`, `bdbcea0` and planning authority `2a88ffe` plus the closed commands-only Verification gate. Remaining separately planned validation branches are browser/manual smoke, API no-DB validation and API DB-backed validation.

The strict-parser chain, all-22-field explicit-null tests-only gate, UI/state stale-data chain, frontend typecheck/build planning authority and frontend typecheck/build Verification execution are all CLOSED. Next eligible branch: exact-path commit/push of this three-file docs/status sync after separate PM authorization; PM handoff if context/thread is long; or separately planned browser/manual smoke, API no-DB or API DB-backed validation.
...
DB/API/Dashboard consumer planning, API consumer contract freeze, API implementation planning, accepted station events API implementation, DB-backed API validation planning/repair, DB-backed API validation execution rerun, Dashboard/API implementation planning, Dashboard implementation preparation / allowlist, Dashboard accepted-events frontend implementation, Dashboard accepted-events vertical validation planning, Dashboard accepted-events no-DB vertical validation execution, Dashboard accepted-events apiClient focused no-DB tests, Dashboard accepted-events nested/renamed leakage defense-in-depth fixture, Dashboard accepted-events strict parser fail-closed hardening, the all-22-field explicit-null regression tests-only gate, the UI/state stale-data planning/tests-only gate and the frontend typecheck/build planning/execution gate are closed. The next eligible branch is exact-path commit/push of this three-file docs/status sync after separate PM authorization, PM handoff if thread/context is long, or separately planned browser/manual smoke, API no-DB or API DB-backed validation.
...
1. 本次 R64 final planning acceptance/status sync 只建立 durable docs 状态；当前为
   `WRITTEN / UNSTAGED / UNCOMMITTED / UNPUSHED`。
2. Static planning review chain 已关闭；不得自动追加 Architecture、Reliability、Data Quality
   或 Verification review。
3. 下一 eligible branch 是单独授权的 future execution-preparation planning，必须冻结
   attempt、roots、builder/candidate、materialization、base resolution、Docker/network
   budget、terminal paths、isolated validation、stop/retry/cleanup 与 Git authority。
4. execution-preparation planning 被 PM 接受后，才可由用户单独授权 actual local
   build/image acceptance；该授权不得自动包含 archive、transport、remote load、deployment、
   activation、runtime A–H 或 production accepted-fact。
5. 用户单独授权后，可仅对 `docs/current_status.md`、`docs/roadmap.md` 与 R64 执行
   exact-path docs-only stage/commit/push；不得 broad-stage `docs/`、R56–R63 或 Batch D/E。
6. 本状态同步不授权 fixture implementation、tests、build、Docker、network、remote、Git、
   deployment、restart、activation、runtime validation 或 production acceptance。
